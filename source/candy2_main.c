...
void actualiza_contadores(unsigned char lev, short p, unsigned char m,
											unsigned char g)
{
	printf("\x1b[38m\x1b[4;0H  nivel: \x1b[39m%d", lev);
	printf("\x1b[38m\x1b[10;0H  Factor Escalado: \x1b[39m%d", factorEscalado);
}
...
/* procesa_sugerencia(mat,lap): según el valor del parámetro lap (número de
	...
	dichas posiciones.
*/
void procesa_sugerencia(char mat[][COLUMNS], unsigned short lap, unsigned char init)
{
	if (lap == T_INACT) 
	...
	{							// activa mostrar elementos sugeridos
		oculta_elementos(mat, pos_sug, init);
		escribe_matriz(mat);
		init = (init > 0) ? 1:init;
		muestra_elementos(mat, pos_sug, init);
		escribe_matriz(mat);
	}
}
...
int main(void)
{
	...
	printf("\x1b[39m\x1b[5;0H  Pulse '\x1b[36mX\x1b[39m' para augmentar");
	printf("\x1b[39m\x1b[6;0H  Pulse '\x1b[36mY\x1b[39m' para reducir");
	printf("\x1b[39m\x1b[7;0H  Pulse '\x1b[36mEnter\x1b[39m' para jugar");
	printf("\x1b[39m\x1b[8;0H  Pulse '\x1b[36mQ\x1b[39m' para escalado++");
	printf("\x1b[39m\x1b[9;0H  Pulse '\x1b[36mW\x1b[39m' para escalado--");
						...
						{	lapse++;				// cuenta tiempo (VBLs) de inactividad
							if (lapse >= T_INACT) {	// a partir de cierto tiempo de inactividad,
								while (!mov) {
									...
									if(keysHeld() & KEY_X) {
										init = 1;
										procesa_sugerencia(matrix, lapse, init);
									}
									if(keysHeld() & KEY_Y) {
										init = 0;
										procesa_sugerencia(matrix, lapse, init);
									...
									actualiza_contadores(level, 0, 0, 0);
								}
							}
						}
#ifdef TRUCOS
