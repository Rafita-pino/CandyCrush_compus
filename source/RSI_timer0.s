...
@;Tarea 2Ga
		...
		beq .L_ignore			@;sÃ­, no actualitzar gelatines
		...
		cmp r5, #0	
		bgt .L_end				@;si > 0, seguent posiciÃ³
		tst r5, #0x80			@;comparaciÃ³ bit mes alt (Ca2)
		bne .L_end				@;si no es 0 (es negatiu), seguent posiciÃ³
		...
		add r4, #GEL_TAM		@;desplaÃ§ament seg. posiciÃ³ mapa gelatines
		cmp r2, #COLUMNS
...
push {r0-r6, lr}
		...
		.L_vect:
			...
			subhi r2, #30			@; si aÃ±adimos 30 al divisor de frecuencia, tardara 136 repeticiones en llegar al limite (suficiente)
			rsb r2, r2, #0
